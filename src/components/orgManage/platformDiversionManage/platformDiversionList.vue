<template>
	<div class="diversion">
		...
		<el-row class="product-list">
			<el-table class="list-table" :data="dataList" highlight-current-row border style="width: 100%"
				...
				<el-table-column prop="type" label="导流名称" header-align="center">
					<template scope="scope">
						<span style="margin-left: 10px">{{ scope.row.deversionName }}</span>
					</template>
				</el-table-column>
		...
		<el-dialog title="新增平台产品关系" :visible.sync="showDialog" style="text-align: left;" size="tiny">
			<el-form :model="formData" :rules="formRules" ref="ruleForm" label-width="130px">
				<el-row>
					<el-form-item prop="data_source_id" label="请选择平台:">
						<el-select placeholder="请选择" value-key="id" v-model="formData.plat">
							<el-option v-for="item in platformList" :key="item.id" :label="item.platName"
									   :value="item"></el-option>
						</el-select>
					</el-form-item>
					<el-form-item prop="data_source_id" label="请选择导流:">
						<el-select placeholder="请选择" value-key="id" v-model="formData.deversion">
							<el-option v-for="item in diversionList" :key="item.id" :label="item.diversionName"
									   :value="item"></el-option>
						</el-select>
					</el-form-item>
				</el-row>
	...
	methods: {
		...
		this.$refs.ruleForm.validate((valid) =>{
		  if (valid) {
			if (this.showDialogType) {//新增
			  this.httpRequest.addPlatformDiversionRelation({
				deversionName: this.formData.deversion.diversionName,
				deversionId: this.formData.deversion.id,
				platId: this.formData.plat.id,
				platName: this.formData.plat.platName
			  }).then((res) =>{
	  ...
	  detele(id){
		this.$confirm('确定要删除此导流吗?', '提示', {
		  ...
		  this.httpRequest.deletePlatformDiversionRelation({
			id: id
		  }).then((res) =>{
	  ...
	  getList(page = 1){
		this.httpRequest.getPlatformDiversionRelationList({
		  page: page,
		  row: 10,
