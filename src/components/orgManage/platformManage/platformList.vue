		...
		<el-row class="product-list">
			<el-table class="list-table" :data="diversionList" highlight-current-row border style="width: 100%"
				...
				<el-table-column prop="type" label="导流编码" header-align="center">
					<template scope="scope">
						<span style="margin-left: 10px">{{ scope.row.platCode }}</span>
					</template>
				</el-table-column>
			...
			<el-form :model="formData" :rules="formRules" ref="ruleForm" label-width="130px">
				<el-row>
					<el-form-item prop="platName" label="平台名称:">
						<el-input v-model="formData.platName" placeholder="请输入平台名称"></el-input>
					</el-form-item>
					<el-form-item prop="platCode" label="平台编码:">
						<el-input v-model="formData.platCode" placeholder="请输入平台编码"></el-input>
					</el-form-item>
					<el-form-item prop="ratio" label="卡量百分比:">
						<el-input v-model="formData.ratio" placeholder="请输入卡量百分比(范围0--100)" type="number"></el-input>
					</el-form-item>
					<el-form-item prop="isValid" label="卡量开关:">
					...
					<!--<el-form-item prop="isValid" label="卡量开关:">
						<el-input v-model="formData.isValid" placeholder="请选择"></el-input>
  ...
  export default {
	data(){
	  return {
		...
		isValidList:[{name:'开',id:1},{name:'关',id:0}],
		showDialogType: true,
	...
	methods: {
	  ...
	  addDiversion(){
		...
		this.titleType = '新增平台信息'
		this.showDialogType = true
		this.formData.platName = ''
		this.formData.platCode = ''
	  },
	  //修改
	  compile(row){
		this.showDialog = true
		this.titleType = '修改平台信息'
		this.showDialogType = false
		this.formData.platName = row.platName
		this.formData.platCode = row.platCode
		this.formData.id = row.id
		this.formData.ratio = row.ratio
		this.formData.isValid = row.isValid
	  },
	  //保存
	  saveData(){
		this.$refs.ruleForm.validate((valid) =>{
		  if (valid) {
			if (this.showDialogType) {//新增
			  this.httpRequest.addPlatform(this.formData).then((res) =>{
			  ...
			  this.httpRequest.updatePlatform(this.formData).then((res) =>{
	  ...
	  detele(id){
		this.$confirm('确定要删除此导流吗?', '提示', {
		  ...
		  this.httpRequest.deletePlatform({
			id: id
		  }).then((res) =>{
